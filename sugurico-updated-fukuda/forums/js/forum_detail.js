...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        if (currentUser) {
            ...
            commentFormContainer.innerHTML = `<p><a href="../../auth/html/login.html">ログイン</a>してコメントを投稿する</p>`;
        }
    ...
    function renderPost(post, tags, images, isPremium, isBookmarked, isOwner) {
        ...
        if (!currentUser) {
            ...
            authorHTML = `<a href="../../auth/html/mypage.html">${authorHTML}</a>`;
        }
        ...
        if (currentUser && post.user_id_auth !== currentUser.id) {
            blockButtonHTML = `
                <div class="block-action">
                    <button type="button" id="block-user-button" class="action-button delete-button" data-target-user-id="${post.user_id_auth}">
                        このユーザーをブロック
                    </button>
                </div>`;
        }
